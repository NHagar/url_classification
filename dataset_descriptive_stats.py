...
class DatasetDescriptiveStats:
    ...
    def analyze_dataset(self, dataset_name: str) -> Dict:
        ...
        if self._has_domain_info(df, dataset_name):
            ...
            stats_dict.update({
                ...
                'domain_distribution': self._analyze_domain_distribution(df, domain_col),
                'domain_skew': self._calculate_domain_skew(df, domain_col),
                'category_per_domain': self._analyze_category_per_domain(df, domain_col),
                'domain_category_cross_table': self._create_domain_category_crosstab(df, domain_col)
            })
        else:
            stats_dict.update({
                'domain_column': None,
    ...
    def _has_domain_info(self, df: pd.DataFrame, dataset_name: str) -> bool:
        ...
        if dataset_name == 'uci':
            ...
            return any(col in df.columns for col in ['domain', 'publisher', 'source', 'website'])
        return False
    ...
    def _calculate_basic_stats(self, df: pd.DataFrame) -> Dict:
        ...
        text_cols = [col for col in df.columns if col.lower() in ['text', 'title', 'content', 'body']]
        if text_cols:
    ...
    def _calculate_domain_skew(self, df: pd.DataFrame, domain_col: str) -> Dict:
        ...
        top_5_percent = domain_counts.head(max(1, len(domain_counts) // 20)).sum() / total_records
        top_10_percent = domain_counts.head(max(1, len(domain_counts) // 10)).sum() / total_records
    ...
    def _analyze_category_per_domain(self, df: pd.DataFrame, domain_col: str) -> Dict:
        ...
        for domain in df[domain_col].unique():
            ...
            domain_category_stats[domain] = {
                ...
                'dominant_category_percent': category_dist.iloc[0] if len(category_dist) > 0 else 0,
                'entropy': stats.entropy(category_counts.values, base=2) if len(category_counts) > 0 else 0,
        ...
        dominant_percents = [stats['dominant_category_percent'] for stats in domain_category_stats.values()]
        single_category_domains = sum([stats['is_single_category'] for stats in domain_category_stats.values()])
        ...
        return {
            ...
            'summary': {
                ...
                'single_category_domains_percent': single_category_domains / len(domain_category_stats)
            }
        }
    
    def _create_domain_category_crosstab(self, df: pd.DataFrame, domain_col: str) -> Dict:
    ...
    def _perform_chi2_test(self, df: pd.DataFrame, domain_col: str, category_col: str) -> Dict:
...
def format_stats_report(all_stats: List[Dict], output_file: Optional[str] = None) -> str:
    ...
    for stats in all_stats:
        ...
        for key, value in basic.items():
            if key.endswith('_length_stats'):
                ...
                report_lines.append(f"  Q25-Q75: {value['q25']:.1f} - {value['q75']:.1f}")
                report_lines.append("")
        ...
        if 'error' not in cat_dist:
            ...
            report_lines.append(f"  Categories: {', '.join(map(str, cat_dist['categories']))}")
            report_lines.append(f"  Most Common: {cat_dist['most_common']} ({cat_dist['proportions'][cat_dist['most_common']]:.1%})")
            report_lines.append(f"  Least Common: {cat_dist['least_common']} ({cat_dist['proportions'][cat_dist['least_common']]:.1%})")
        ...
        if stats['domain_column']:
            ...
            report_lines.append(f"  Domain Record Statistics:")
            domain_stats = domain_dist['domain_counts_stats']
            report_lines.append(f"    Mean: {domain_stats['mean']:.1f} records/domain")
            report_lines.append(f"    Median: {domain_stats['median']:.1f} records/domain")
            report_lines.append(f"    Std Dev: {domain_stats['std']:.1f}")
            report_lines.append(f"    Range: {domain_stats['min']:.0f} - {domain_stats['max']:.0f}")
            ...
            report_lines.append(f"  HHI (Concentration): {skew['herfindahl_hirschman_index']:.3f}")
            report_lines.append(f"  Effective # of domains: {skew['effective_number_domains']:.1f}")
            report_lines.append(f"  Top domain share: {skew['top_1_domain_percent']:.1%}")
            report_lines.append(f"  Top 5 domains share: {skew['top_5_domains_percent']:.1%}")
            report_lines.append(f"  Top 10 domains share: {skew['top_10_domains_percent']:.1%}")
            report_lines.append(f"  Single-record domains: {skew['domains_with_single_record']} ({skew['domains_with_single_record']/domain_dist['n_domains']:.1%})")
            report_lines.append(f"  Domains with <10 records: {skew['domains_with_lt_10_records']} ({skew['domains_with_lt_10_records']/domain_dist['n_domains']:.1%})")
            report_lines.append(f"  Domains with >100 records: {skew['domains_with_gt_100_records']} ({skew['domains_with_gt_100_records']/domain_dist['n_domains']:.1%})")
            report_lines.append("")
            ...
            if 'error' not in cat_per_domain:
                ...
                report_lines.append(f"  Average entropy per domain: {summary['avg_entropy_per_domain']:.3f} ± {summary['std_entropy_per_domain']:.3f}")
                report_lines.append(f"  Average dominant category %: {summary['avg_dominant_category_percent']:.1%} ± {summary['std_dominant_category_percent']:.1%}")
                report_lines.append(f"  Single-category domains: {summary['single_category_domains']} ({summary['single_category_domains_percent']:.1%})")
                
                # Show examples of highly skewed domains
                report_lines.append("  Most category-skewed domains (top 10):")
                domain_skews = [(domain, stats_dict['dominant_category_percent']) 
                ...
                for domain, skew_pct in domain_skews[:10]:
                    ...
                    report_lines.append(f"    {domain}: {skew_pct:.1%} {dominant_cat} ({total_records} total records)")
                report_lines.append("")
                ...
                if 'chi2_test' in crosstab and 'error' not in crosstab['chi2_test']:
                    ...
                    report_lines.append(f"  Chi-square statistic: {chi2_results['chi2_statistic']:.2f}")
                    report_lines.append(f"  p-value: {chi2_results['p_value']:.2e}")
                    report_lines.append(f"  Degrees of freedom: {chi2_results['degrees_of_freedom']}")
                    report_lines.append(f"  Significant association: {'Yes' if chi2_results['is_significant'] else 'No'}")
                    report_lines.append(f"  Cramer's V (effect size): {chi2_results['cramers_v']:.3f}")
                    report_lines.append("")
    ...
    if len(all_stats) > 1:
        ...
        comparison_table = []
        headers = ["Dataset", "Records", "Categories", "Domains", "Gini Coeff", "Top Domain %", "Single-Cat Domains %"]
        ...
        for stats in all_stats:
            ...
            if stats['domain_column']:
                ...
                if 'error' not in stats['category_per_domain']:
                    row.append(f"{stats['category_per_domain']['summary']['single_category_domains_percent']:.1%}")
                else:
                    row.append("N/A")
            else:
        ...
        # Format table
        col_widths = [max(len(str(row[i])) for row in [headers] + comparison_table) for i in range(len(headers))]
...
def main():
    parser = argparse.ArgumentParser(description="Calculate descriptive statistics for news classification datasets")
    parser.add_argument(
        "--datasets", 
        nargs="+", 
